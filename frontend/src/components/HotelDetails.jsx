...
import TopBar from "./hoteldetails/TopBar"; // Ensure the path is correct
...
import './hoteldetails/hoteldetails.css'; // Import the CSS file
...
const HotelDetails = () => {
  ...
  const { destinationId, startDate, endDate, guests, rooms, enhancedHotels } = location.state || {};
  ...
  useEffect(() => {
    const fetchHotelDetails = async () => {
      try {
        //using json file downloaded from the api endpoint for now
        const [detailsResponse, pricesResponse] = await Promise.all([
          fetch('/hotelDetails.json'),
          fetch('/hotelDetailsPrice.json')
        ]);
        const detailsData = await detailsResponse.json();
        const pricesData = await pricesResponse.json();
        setRoomDetails(pricesData.rooms);
